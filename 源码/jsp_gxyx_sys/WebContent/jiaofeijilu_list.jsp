...
<body>
	...
	<form name="form1" id="form1" action="">
		搜索:&nbsp;&nbsp;学号：<input name="xuehao" type="text" id="xuehao"
			...
			style='border: solid 1px #000000; color: #666666' size='15' />&nbsp;&nbsp;班级：<input
			name="banji" type="text" id="banji"
			style='border: solid 1px #000000; color: #666666' size='15' />&nbsp;&nbsp;专业：<input
			name="zhuanye" type="text" id="zhuanye"
			style='border: solid 1px #000000; color: #666666' size='15' /> 学年： <select
			name='xuenian' id='xuenian'>
	...
	<table width="100%" border="1" align="center" cellpadding="3"
		...
		<%
			...
			if (((curpage - 1) * page_record) == 0 || curpage == 0) {
				...
				String tempsql = "select  id from jiaofeijilu order by id desc limit 0," + (curpage - 1) * page_record
						+ "";
				...
				sql = "select  * from jiaofeijilu where id not in (" + tempidstring + ")  ";
			}
		...
		<tr>
			...
			<td width='80' align='center'><a
				href="sh.jsp?id=<%=id%>&yuan=<%=issh%>&tablename=jiaofeijilu"
				onClick="return confirm('您确定要执行此操作？')"><%=issh%></a></td>
			<td width="138" align="center"><%=addtime%></td>
			<td width="60" align="center"><a
				href="jiaofeijilu_updt.jsp?id=<%=id%>">修改</a> <a
	...
	<p align="center">
		&nbsp;共<%=zgs%>条记录&nbsp;&nbsp;<%=page_record%>条/页 <a
			href="jiaofeijilu_list.jsp?page=1">首页</a> <a
			href="jiaofeijilu_list.jsp?page=<%=curpage - 1%>">上一页</a> <A
			href="jiaofeijilu_list.jsp?page=<%=curpage + 1%>">下一页</A> <a
			href="jiaofeijilu_list.jsp?page=<%=zys%>">尾页</A> 当前第<FONT color=red><%=curpage%></FONT>页/共<FONT
